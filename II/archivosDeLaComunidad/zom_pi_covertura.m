...
grid on;
LG1=legend('Zo_T','Zo_P_i','Zo_P_i_m',1);
...
hold on;
plot(-0.8267,1,'bo');       % Lugar de Zopim = 1
plot(0.8267,1,'bo');       % Lugar de Zopim = 1

plot(-0.866,1.04,'bo');     % Lugar de Zopim(maximo)
plot(0.866,1.04,'bo');     % Lugar de Zopim(maximo)

plot(-0.661,0.96,'bo');     % Lugar de Zopim(minimo)
plot(0.661,0.96,'bo');     % Lugar de Zopim(minimo)
axis([-1,1,0.7,1.4]);
hold on;
L1=line([-1,1],[1,1]);
set(L1,'color','k','linestyle','-','linewidth',1); 

plot(-0.661,0.5,'bo');     % Lugar de Zopim(minimo)
plot(0.661,0.5,'bo');     % Lugar de Zopim(minimo)
line([-1,1],[1.04,1.04]); % Marca Horizontal
line([-1,1],[0.96,0.96]); % Marca Horizontal


line([-0.866,-0.866],[0.5,1.04]); % Marca Horizontal
line([-1,1],[0.96,0.96]); % Marca Horizontal
